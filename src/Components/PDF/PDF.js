...
const PDF = forwardRef(({ pdf, type, language }, ref) => {
  ...
  const [userInfo, setUserInfo] = useState(data[index]);
  //console.log("user", user); // _id name surename father_name bio_img
  const {
    name,
    lastName,
    fatherName,
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          Tizim testlanmoqda! Ushbu arizangiz haqiqiy hisoblanmaydi.
        </h2>
          ...
          >
            <List>
              ...
              >
                {"F . I . SH"} :{" "}
                <span style={{ fontWeight: "bold", width: "60%" }}>
                  {userInfo.name} {"  "} {userInfo.surename} {"  "}{" "}
              ...
              >
                {"Passport"} :{" "}
                <span style={{ fontWeight: "bold", width: "60%" }}>
                  {userInfo.passport_seria_number}
                </span>
              </div>
              <div
                style={type ? { width: "80%" } : { width: "80%", fontSize: 15 }}
              ...
              >
                ...
                <span style={{ fontWeight: "bold", width: "60%" }}>
                  {userInfo.passport_location}
                </span>
              </div>
              {/* <div style={type ? {width: '80%'} : {width: '80%', fontSize: 15}}>
                                {'Tug\'lgan sana'} : <span style={{fontWeight: 'bold', width: "60%"}}>{userInfo.birth_date}</span>
                            </div> */}
              <div
                style={type ? { width: "80%" } : { width: "80%", fontSize: 15 }}
              >
          ...
          <div style={{ width: "90%" }}>
            <List>
              ...
              >
                ...
                <span style={{ fontWeight: "bold" }}>{userInfo.region}</span>
              </List.Item>
              <List.Item style={type ? {} : { fontSize: 16 }}>
                {"Test topshiradigan sana"} :{" "}
                <span style={{ fontWeight: "bold" }}>1-Avgust</span>
              </List.Item>
              <List.Item
                style={
                  type
        ...
        <List>
          ...
          <List.Item style={type ? {} : { fontSize: 16 }}>
            ...
            <span style={{ fontWeight: "bold" }}>
              Yoshlar ittifoqi binosi ko'chasi
            </span>
          </List.Item>
          <div style={{ display: "flex" }}>
            <List.Item
